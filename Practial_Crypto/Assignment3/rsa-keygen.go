...
func main() {
	var modulus, phi, temp1, temp2 = new(big.Int), new(big.Int), new(big.Int), new(big.Int)
	...
	checkForError(err)
	fmt.Println(p.ProbablyPrime(100), q.ProbablyPrime(100), millerRabinPrimalityTest(p), millerRabinPrimalityTest(q))
	fmt.Println(modulus)
	fmt.Println(e)
	fmt.Fprint(f, "(", modulus, ", ", e, ")")
	f.Close()
	f, err = os.Create(os.Args[2])
	checkForError(err)
	fmt.Println(d)
	fmt.Fprint(f, "(", modulus, ", ", d, ", ", p, ", ", q, ")")
	f.Close()
}
